...
async function handle(req, res) {
  ...
  if (url.pathname === '/__edit__/apply-batch' && req.method === 'POST') {
    ...
    for (let i = 0; i < ops.length; i++) {
      ...
      if (typeof op !== 'string' || !Number.isFinite(Number(payload.line))) {
        sendJson(res, 400, { ok: false, message: `第 ${i + 1} 个操作缺少 op / line` });
        return true;
      }
      ...
      // 源块：优先用锚点文本在当前内容中重定位（前面的操作可能已改变行号）
      let line;
      if (item.anchorText) {
        line = findAnchorLine(normBody, item.anchorText, approxBody);
        if (line == null) {
          sendJson(res, 400, { ok: false, message: `第 ${i + 1} 个操作（${op}）的目标块内容已变化，无法定位；请刷新页面后重试` });
          return true;
        }
      } else {
        line = approxBody;
      }
      const p2 = { ...payload, line };

      // 目标（move-block 的 targetLine / insert-into-card 的 targetLine）
      if (payload.targetLine != null) {
        ...
        if (item.targetAnchorText) {
          ...
          if (tl == null) {
            sendJson(res, 400, { ok: false, message: `第 ${i + 1} 个操作（${op}）的目标位置内容已变化，无法定位；请刷新页面后重试` });
            return true;
          }
        } else {
          tl = approxTarget;
      ...
      if (!result.ok) {
        sendJson(res, 400, { ok: false, message: `第 ${i + 1} 个操作（${op}）失败：${result.message}` });
        return true;
      }
    ...
    await fs.writeFile(file, out, 'utf8');
    const stack = history.get(file) || [];
    stack.push({ before, after: out, op: 'batch', payload: { count: applied.length }, time: Date.now() });
    if (stack.length > 50) stack.shift();
    history.set(file, stack);

    sendJson(res, 200, {
      ok: true,
      message: `已保存 ${applied.length} 处修改`,
      file: rel(file),
      savedCount: applied.length,
      undoCount: stack.length,
    });
    return true;
  }
