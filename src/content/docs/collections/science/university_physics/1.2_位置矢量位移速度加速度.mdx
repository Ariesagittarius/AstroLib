...
在直角坐标系中,位矢 r 可以表示成
$$
\boldsymbol{r} = x\boldsymbol{i} + y\boldsymbol{j} + z\boldsymbol{k}\tag{1.1}
$$
式中，$\boldsymbol{i}、\boldsymbol{j}、\boldsymbol{k}$ 分别表示沿 x 轴、y 轴、z 轴正方向的单位矢量。位矢 r 的大小为
$$
|\boldsymbol{r}| = r = \sqrt{x^2 + y^2 + z^2}\tag{1.2}
$$
位矢的方向余弦为
$$
