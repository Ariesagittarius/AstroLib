...
解（1）由动量定理 $\int_0^t F\mathrm{d}t = mv$ ，得
...
解（1）建立坐标系，如图2.29所示.设某时刻链条下垂部分的长度为 $x$ ，则保留在桌面上的长度为 $l - x$ ，剩余长度所受摩擦力 $f = \mu mg(l - x) / l$ ，方向与滑动方向相反.
...
解（1）第一宇宙速度.航天器绕地球运动所需的最小速度，称为第一宇宙速度.以地心为坐标原点，航天器在距地心 $r$ 处绕地球做圆周运动的速度为 $v_{1}$ ，则有
