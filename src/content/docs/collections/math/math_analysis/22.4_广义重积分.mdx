...
<Note title="注">
在上述定义中, 还可将函数 $f$ 在 $D$ 内有一个奇点改为在 $D$ 内有一条奇线 $\gamma$ , 即曲线 $\gamma$ 上每一点都是 $f$ 的奇点. 设 $D_{l}$ 是任意可以将 $\gamma$ 围起来的可求面积区域, 设 $f$ 在 $D \setminus D_l$ 中可积. 令 $D_l$ 收缩为 $\gamma$ , 记为 $D_l \to \gamma$ , 如果极限
...
<Note title="注">
$Poisson$ 积分中 $e^{-x^{2}}$ 的原函数不是初等函数, $Poisson$ 敏锐地观察到极坐标下二重积分有因子 $r \, dr \, d\theta$ , 由此出发他给出了上述巧妙的算法 <sup>①</sup> .
</Note>
</Example>
