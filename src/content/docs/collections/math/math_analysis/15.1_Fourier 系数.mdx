...
<Note title="注 2">
当 $f$ 为偶函数或奇函数时， $f$ 的$Fourier$级数有较为简单的形式：当 $f$ 为偶函数时，所有的 $b_{n}$ 均为 $0$，并且$(15.1)$成为
...
<Note title="注 3">
从公式$(15.4)$不难看出, 如果 $f$ 在 $[0, \pi]$ 上具有某种对称性, 例如关于点 $\left(\frac{\pi}{2}, 0\right)$ 为偶函数 (奇函数), 则可以证明在公式$(15.4)$中当 $n$ 为奇数 (偶数)时积分为 $0$. 对于公式$(15.5)$有类似的结论 (参见上册10.4.3小节中的命题和例题).
</Note>
...
<Note title="注">
在右边的图 $15.1$ 中作出了命题的几何意义的示意图. 如果考虑由所有 $T_{n}$ 构成的集合, 问题就是要从中找出与 $f$ 距离最小的一个三角多项式. 这个问题的解就是 $S_{n}$ . 其原因在于, $Euler-Fourier$ 公式在几何上就是 $f$ 到三角函数系的正交投影, 而 $S_{n}$ 就是 $f$ 到所有 $T_{n}$ 的集合上的正交投影, 因此 $S_{n}$ 到 $f$ 的距离最短.
</Note>
</Knowledge>

<figure class="vp-figure">
  ![](images/43d7b19330081728ff7f312c18c0c84ca1c854abc2699bbb9f06810bf6b36255.jpg)
  <figcaption>图15.1</figcaption>
</figure>
...
<Note title="注 1">
这个结果与$Fourier$级数的收敛性没有关系, 由此就可以得到 $a_{n} = o(1)$ , $b_{n} = o(1)$ , 这里也不需要用$Riemann$引理.
</Note>

<Note title="注 2">
由此可知, 在 $0 < p \leqslant \frac{1}{2}$ 时, 三角级数 $\sum_{n=1}^{\infty} \frac{\sin nx}{n^p}, \sum_{n=1}^{\infty} \frac{\cos nx}{n^p}$ 不可能是可积和平方可积函数的 $Fourier$ 级数①.
</Note>
</Knowledge>
