...
$$
= \left\| T ^ {\prime} (s) \right\| = \left\| r ^ {\prime \prime} (s) \right\|. \quad i
$$
...
意到 $\| r'(s) \| = 1$ ，利用向量积的定义与(7.2)式，由(7.8)式可得
...
<Solution title="解法二">
在圆周上任取两点 $P_{1}, P_{2}$ , 圆在点 $P_{1}$ 与 $P_{2}$ 切线的夹角 $\Delta \theta$ 等于 $OP_{1}$ 与 $OP_{2}$ 所夹的圆心角, 其中 $O$ 为圆心. 于是弧长
...
$$
\kappa = \frac {a}{a ^ {2} + k ^ {2}}. \quad \text {   II   }
$$
</Solution>
</Example>
...
意到 $\boldsymbol{\rho}'(s)$ 是渐伸线 $\Gamma$ 的切向量, 由渐伸线的定义知
