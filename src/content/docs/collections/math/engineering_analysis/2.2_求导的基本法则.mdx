...
（导数的有理运算法则）设函数 $u, v: I \rightarrow R$ 在 $x \in I$ 可导，则它们的和、差、积、商（分母为零的点除外）也在 x 可导，且
...
（链式法则）设函数 $u=g(x)$ 在 x 处可导，函数 $y=f(u)$ 在与 x 相对应的 u 处可导，则复合函数 $y=f[g(x)]$ 在 x 处可导，并且
...
\frac {\mathrm{d} y}{\mathrm{d} x} = f ^ {\prime} (u) g ^ {\prime} (x) = \frac {\mathrm{d} y}{\mathrm{d} u} \cdot \frac {\mathrm{d} u}{\mathrm{d} x}. \quad \text {   Ⅱ   }
...
（反函数求导法则）设区间 I 上的严格单调连续函数 $x=f(y)$ 在点 y 处可导，且 $f'(y) \neq 0$ ，则它的反函数 $y=f^{-1}(x)$ 在对应点 x 处可导，并且
