...
(点列的极限) 设 $\{x_{k}\}$ 是 $R^{n}$ 中的一个点列, 其中 $x_{k}=(x_{k,1},x_{k,2},\cdots,x_{k,n})$ , 又设 $a=(a_{1},a_{2},\cdots,a_{n})$ 是 $R^{n}$ 中的一固定点, 若当 $k\to\infty$ 时, $\rho(x_{k},a)\to0$ , 即
...
(Bolzano-Weierstrass 定理) $R^{n}$ 中的有界点列必有收敛子列. ( $R^{n}$ 中点列 $\{x_{k}\}$ 的收敛子列的极限也称为 $\{x_{k}\}$ 的极限点.)
...
(Cauchy 收敛原理) $R^{n}$ 中点列 $\{x_{k}\}$ 收敛于 $R^{n}$ 中的点的充要条件为 $\{x_{k}\}$ 是 $R^{n}$ 中的 Cauchy 点列.
...
\*证 必要性 设 $A$ 是开集, 故 $A^{\circ} = A$ . 为了证明 $A^{c}$ 是闭集, 只要证明 $(A^{c})' \subseteq A^{c}$ . 若 $(A^{c})' = \emptyset$ , 则显然有 $(A^{c})' \subseteq A^{c}$ . 若 $(A^{c})' \neq \emptyset$ , 设 $x \in (A^{c})'$ , 则 $\forall \varepsilon > 0$ ,
...
1. 设 $\{x_{k}\}$ 为 $R^{n}$ 中的点列, $a \in R^{n}, \lim_{k \to \infty} x_{k} = a$ , 证明: $\lim_{k \to \infty} \| x_{k} \| = \| a \|$ .

2. 求平面 $R^{2}$ 中下列点列的极限(其中 $n \in N_{+}$ ):

(1) $\left(\frac{(-1)^{n}}{n},\frac{n}{n-1}\right)$ ; (2) $\left(\frac{n^{2}+1}{n^{2}-n-1},\left(1+\frac{1}{n}\right)^{n}\right)$ .

3. 证明定理 1.2 中的(2)，(4).

4. 求下列各集的导集、闭包，并说明是否为闭集：

(1) $A=\left\{(x,y)\mid x^{2}+y^{2}>2\right\}$ ; (2) $A=\left\{\left(\frac{1}{m},\frac{1}{n}\right)\mid m,n\in\mathbf{N}_{+}\right\}$ ;

(3) $A=\left\{(x,y)\mid x,y\text{为整数}\right\}$ ; (4) $A=\left\{(x,y)\mid x,y\text{为有理数}\right\}$ .

5. 下列集合是开集还是闭集, 求出它们的内部、边界和闭包:

(1) $A=\left\{(x,y)\in\mathbf{R}^{2}\mid x\geqslant0,y\geqslant0,x+y\leqslant1\right\}$ ; (2) $A=\left\{(x,y)\in\mathbf{R}^{2}\mid y<x^{2}\right\}$ ;

(3) $A=\left\{x\in\mathbf{R}^{2}\mid\|x\|=1\right\}$ ; (4) $A=\left\{(x,y)\in\mathbf{R}^{2}\mid-1<x<1,y=0\right\}$ .

6. 第5题中的集合是否为区域？有界还是无界？

7. 说明下列集合是紧集：

(1) 有限点集；

(2) $A=\left\{0,1,\frac{1}{2},\frac{1}{3},\cdots\right\}$ ;

(3) $R^{n}$ 中的闭区间；

(4) $R^{n}$ 中的单位球面 $\{x \in R^{n} \mid \|x\| = 1\}$ .

(B)

1. 设 $A \subseteq \mathbf{R}^n$ 是一个点集. 证明：

(1) $A^{\circ}$ 与 ext A 是开集；

(2) $A'$ , $\partial A$ 是闭集;

(3) A 为开集 $\Leftrightarrow A \cap \partial A = \varnothing.$

2. 以 n=2 为例证明聚点原理: $R^{n}$ 中的有界无限点集至少有一个聚点.
</Block>
