...
<Solution title="解法三">
$\int \csc x\mathrm{d}x = \int \frac{\csc x(\csc x + \cot x)}{\csc x + \cot x}\mathrm{d}x$ $= -\int \frac{\mathrm{d}(\csc x + \cot x)}{\csc x + \cot x}\\ = -\ln |\csc x + \cot x| + C.$
</Solution>
</Example>
...
<QRCodeVideo id="3.3.1" title="两类换元法的" url="http://2d.hep.cn/1254051/36" />
</Solution>
</Example>
...
<QRCodeVideo id="3.3.2" title="定积分换元法与不定积分换元法的区别." url="http://2d.hep.cn/1254051/37" />
