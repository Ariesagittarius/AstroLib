...
大家知道，如果细棒上物质的分布是均匀的，就是说细棒上单位长度的质量都相等，那么各点的线密度相同，只要通过除法用公式 $\rho = \frac{M}{l}$ 就能求得，其中 $M$ 为细棒的总质量。现在细棒上的物质是非均匀分布的，细棒上不同部位处单位长度的质量不一定相等，自然不能用上述公式来计算细棒上各点的线密度。

为了求出细棒上各点的线密度，以细棒的一端为原点，细棒所在直线为 $x$ 轴建立坐标系（图 2.1），则从原点到点 $x$ 一段细棒的质量为非线性函数：
$$
m = m(x), \quad x \in [0, l]
$$
...
$$
在微小区间 $[x_0, x_0 + \Delta x]$ 内，质量的变化可近似看作是均匀的，用除法求得的平均线密度：
$$
\bar{\rho} = \frac{\Delta m}{\Delta x} = \frac{m(x_0 + \Delta x) - m(x_0)}{\Delta x}
$$
就是点 $x_0$ 处线密度的近似值。显然，$\Delta x$ 越小，这个近似值越精确。所以，细棒在点 $x_0$ 处的线密度 $\rho(x_0)$ 应是当 $\Delta x \to 0$ 时平均线密度的极限，即：
$$
