...
<QRCodeVideo id="2.6.1" title="用导函数在一点的正负能判定该点邻域内函数的单调性吗？" url="http://2d.hep.cn/1254051/29" />
...
<QRCodeVideo id="2.6.2" title="函数在极值点的左右邻域内一定单调吗？" url="http://2d.hep.cn/1254051/51" />
...
解方程 $f'(x)=0$ ，得f的驻点 $x=\frac{1}{2}$ 。又f没有不可导点，且 $f\left(\frac{1}{2}\right)=2^{1-p},f(0)=1$ ，$f(1)=1$ , 比较得知 f 在 $[0,1]$ 上的最小值为 $2^{1-p}$ , 最大值为 1.
...
解方程 $\frac{dS}{dR}=0$ ,得驻点 $R=\sqrt[3]{\frac{V}{\pi}}$ .又
...
<figure class="vp-figure">
  ...
  <figcaption>图2.17</figcaption>
</figure>

<Knowledge title="定义 6.1 函数图像的凹凸性">
设函数 $y=f(x)$ 两点 $x_{1}$ 与 $x_{2}$ ，曲线 $y=f(x)$ 上相应的弧段 $\widehat{P_{1}P_{2}}$ 始终位于弦 $\overline{P_{1}P_{2}}$ 的下（上）方（图 2.17），则称函数 $f(x)$ 在区间 I 中的图像是凹（凸）的，也称曲线 $y=f(x)$ 是凹（凸）的.
</Knowledge>
...
<QRCodeVideo id="2.6.3" title="利用导数证明" url="http://2d.hep.cn/1254051/30" />
