...
<Note>
到上述积分的被积函数为一个奇函数,所以其积分值等于0,即 $E(U)=0$ .又因为 $X=\mu+\sigma U$ ,所以由数学期望的性质得
...
<Knowledge title="定理 2.5.2">
(指数分布的无记忆性) 如果随机变量 $X \sim Exp(\lambda)$ ，则对任意 s > 0, t >
...
<Note>
到两次故障之间的时间间隔 $T$ 是非负随机变量，且事件 $\{T \geqslant t\}$ 说明此设备在 $[0, t]$ 内没有发生故障，即 $\{T \geqslant t\} = \{N(t) = 0\}$ ，由此我们得
