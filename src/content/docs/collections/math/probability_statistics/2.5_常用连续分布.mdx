...
<figure class="vp-figure">
  ...
  <figcaption>图2.5.1 正态分布</figcaption>
</figure>
...
<figure class="vp-figure">
  ...
  <figcaption>图2.5.2 正态密度函数</figcaption>
</figure>
$$
...
若随机变量 X 的密度函数为
$$
p (x) = \left\{ \begin{array}{l l} \lambda \mathrm{e} ^ {- \lambda x}, & x > 0, \\ 0, & x \leqslant 0, \end{array} \right.
$$
则称 $X$ 服从参数为 $\lambda$ 的指数分布, 记作 $X \sim \operatorname{Exp}(\lambda)$ 或 $X \sim \mathrm{e}(\lambda)$ .
指数分布的分布函数为
$$
F (x) = \left\{ \begin{array}{l l} 1 - \mathrm{e} ^ {- \lambda x}, & x > 0, \\ 0, & x \leqslant 0. \end{array} \right.
$$
