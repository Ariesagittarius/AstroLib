...
### 2. 关键 Prompt 约束规则一览
1. **实质性学术汇总要求**（防敷衍与纯罗列）：明确禁止仅罗列检索命中项、来源清单或章节目录（如“命中X条”、“相关内容在第 X 章”等）。要求 AI 必须开门见山给出实质性核心结论，并展开数学公式推导、成立条件、概念内涵或典型计算步骤，做到言之有物、逻辑严谨。
2. **专业学科知识兜底**：当书内检索片段无法完全覆盖问题细节时，严禁抛出碎片或直接推诿，要求 AI 基于专业导师底蕴直接给出标准、严密的学术解答与推导，并在文末指引书中相近章节。
3. **站内绝对路径超链接**：要求引用具体知识点时在正文中自然融入 `[标题](/collections/col/book/...#anchor)` 形式的 markdown 链接（利用片段与工具结果返回的 `url`），让来源服务于论证，绝不将链接堆砌成清单。
4. **KaTeX 渲染兼容**：行内公式统一采用 `$..$`，块级公式统一采用 `$$..$$`。
5. **脚注引用对齐**：仅在 `discussion=false` 时要求模型在句末标注 `[1]`..`[n]` 上标，对应 `buildContext` 提供的 TopK 片段。
6. **优雅的未作答兜底**：若大模型耗尽轮次未能产出最终文本，呈现清爽明了的“【AI 本轮未能输出最终回答】”提示，引导查阅下方来源卡片，彻底避免原始调试与命中日志外泄。
