 ...
 * 全站行为开关。
 *
 * refs：正文引用徽章（“例题 1.74”“图 3-48”等）的联动模式：
 *   - 'interactive'（默认）：徽章可点击 / 悬停联动，同页平滑滚动、跨页跳转。
 *     相关处理（静态徽章升级、事件挂接、全局索引解析）已全部移到页面绘制后的
 *     空闲时段执行，切换文章依然流畅；
 *   - 'static'：所有引用统一为构建期生成的静态 chip，不做任何事件挂接，
 *     也不输出跨页索引（页面 HTML 更小、切换开销最小）；代价是失去点击跳转能力。
 *
