...
async function loadBookMeta() {
  ...
  for (const cfg of ['collections.config.mjs', 'books.config.mjs']) {
    try {
      const mod = await import(pathToFileURL(path.join(ROOT, 'src', 'config', cfg)).href);
      for (const col of mod.collections || []) {
        for (const book of col.books || []) merge(col.slug, book);
      }
    } catch (e) {
      console.warn(`[epub] 读取 ${cfg} 失败：`, e.message);
    }
  }
  return meta;
}
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  ...
  <div class="title-page">
    ...
    <p class="book-meta">数字化 EPUB 版 · 由「教辅数字化智能智库」生成</p>
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="book-id" xml:lang="zh-CN">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    ...
    <dc:creator>教辅数字化智能智库</dc:creator>
    <dc:publisher>教辅数字化智能智库</dc:publisher>
